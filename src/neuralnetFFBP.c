...
{
	// actvate the first layer once the input is taken
	activate_layer( nn->use_layer + 0 );
...
{
	// calculate output_gradient and weight_gradient
	deactivate_layer( nn->use_layer + nn->layer_count - 1 );
	for(ulli i=nn->layer_count-1;i>0;i--)
	{
		...
		deactivate_layer( nn->use_layer + i - 1 );
